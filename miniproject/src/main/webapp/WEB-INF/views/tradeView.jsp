...
<button
		onclick="location.href='/miniproject/board/trade/write?pgNum=${requestScope.pgNum}';"
		id="btn1" data-transition=“fade” ><i class="fas fa-pencil-alt"></i>WRITE</button>
	<div class="trade">
		<table>
			<tr>
				<th><i class="fas fa-list-ol"></i></th>
				<th><i class="fas fa-align-center"></i></th>
			...
			<tr>
				<td><%=vo.getId()%></td>
				<td><a
					href="/miniproject/board/trade/read?id=<%=vo.getId()%>&pgNum=${requestScope.pgNum}"><%=vo.getTitle()%></a></td>
				<td><a
					href="/miniproject/board/trade/writer?writer=<%=vo.getWriter()%>&pgNum=${requestScope.pgNum}"><%=vo.getWriter()%></a></td>
	...
	<a href="/miniproject/board/trade?pgNum=${pageStart}">&laquo;</a>

	<c:if test="<%=preData%>">
		<a href="/miniproject/board/trade?pgNum=${requestScope.pgNum-1}">이전</a>
	</c:if>

	<c:forEach var="i" begin="${pageStart}" end="${pageEnd}">
		<a style="${i == requestScope.pgNum? 'color:red;' : 'color : black;'}"
			href="/miniproject/board/trade?pgNum=${i}">${i} &nbsp;</a>
	</c:forEach>

	<c:if test="<%=nextData%>">
		<a href="/miniproject/board/trade?pgNum=${requestScope.pgNum+1}">다음</a>
	</c:if>

	<a href="/miniproject/board/trade?pgNum=${pageEnd}">&raquo;</a>

	<br />
	<br />
	<hr>
	<form method="get" action="/miniproject/board/trade/search" id="search">
